...
class TrajectoryGenerator {
  ...
  * T - the desired time in seconds at which we will be at the goal (relative to
  * now as t=0)
  *
  * OUTPUT
  ...
  * this trajectory.
  */
  trajectory PTG(std::vector<double> start_state, std::vector<double> end_state,
                 double T);
  ...
  */
  ...
  /**
  * Get host state {s, s_dot, s_ddot, d, d_dot, d_ddot} based on previous path
  * size.
  */
  ...
  */
  std::vector<double> trapezoidal_accel(double t_horizon, double v_goal,
  ...
  */
  std::vector< std::vector<double> > get_points_for_goal(
      std::vector<double> goal_state, double t_horizon, int prev_size);
  ...
  */
  ...
  // last element of return vectory is T
  std::vector<double> perturb_goal(std::vector<double> start_state,
                                   std::vector<double> end_state, double T,
                                   double sigma_multiplier);
